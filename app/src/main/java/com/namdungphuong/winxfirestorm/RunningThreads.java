...
public class RunningThreads implements Runnable {
	...
	public void run() {
		...
		if(wordClass.flag){
	        ...
	        //for(int i=0;i<GameSpace.poss.length;i++) Log.d("poss1",GameSpace.poss[i]);
	        //for(int i=0;i<wordClass.possibilities.size();i++) Log.d("poss2",wordClass.possibilities.get(i));
